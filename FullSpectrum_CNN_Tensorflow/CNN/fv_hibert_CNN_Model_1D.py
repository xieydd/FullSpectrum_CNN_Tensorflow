# -*- coding: utf-8 -*-
"""
Created on Tue Jan  9 15:16:36 2018

E-mail: xieydd@gmail.com

@author: xieydd

@description:一维全矢卷积神经网络模型
"""
import os  
import numpy as np  
...
class FV_Hilbert_CNNConfig(object):
    ...
    signal_length = 3036    #信号长度
    signal_classes = 9          #信号种类
    
    signal_reshaped = 1024         #
...
class FV_Hilbert_CNN(object):
    ...
    def cnn(self):
        ...
        with tf.variable_scope('layer1-conv1'):
            ...
            #TODO
            conv1 = tf.nn.conv1d(self.input_x,conv1_weights,stride=1,padding="VALID")
            relu1 = tf.nn.relu(tf.nn.bias_add(conv1,conv1_biases))
        with tf.variable_scope('layer2-pool1'):
            pool1 = tf.layers.max_pooling1d(relu1,pool_size=self.config.num_pool1,strides=2,padding="VALID")
            
        with tf.variable_scope('layer3-conv2'):
            conv2_weights = tf.get_variable("weights",[self.config.kernel_size2,self.config.channels2,self.config.kernel_num2],initializer = tf.truncated_normal_initializer(stddev=0.1))
            conv2_biases = tf.get_variable("biases",[self.config.kernel_num2],initializer=tf.constant_initializer(0.0))
            conv2 = tf.nn.conv1d(pool1,conv2_weights,stride=1,padding="VALID")
            relu2 = tf.nn.relu(tf.nn.bias_add(conv2,conv2_biases))

        with tf.variable_scope("layer4-pool2"):
            self.pool2 = tf.layers.max_pooling1d(relu2,pool_size=self.config.num_pool2,strides=2,padding="VALID")
